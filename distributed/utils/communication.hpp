#ifndef UTILS_COMMUNICATION_HPP_
#define UTILS_COMMUNICATION_HPP_
...
void pregel_recv(void* buf, int size, int src, unsigned tag = 0) {
	MPI_Recv(buf, size, MPI_CHAR, src, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
...
obinstream recv_obinstream(int src, unsigned tag = 0) {
	size_t size;
	pregel_recv(&size, sizeof(size_t), src, tag);
...
#endif /* UTILS_COMMUNICATION_HPP_ */
